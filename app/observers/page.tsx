...
import { REGIONS, type ObserverView } from "@/lib/sonar-data";
...
export default function ObserversPage() {
  ...
  const source = snapshot?.source ?? "placeholder";
  ...
  return (
    <PageFrame wide>
      <PageIntro
        ...
        title="Observer registry"
        description="Every active observer is visible, stake-backed, and traceable to a region-level view of network health."
        aside={<StatusPill active={source === "onchain"}>{source === "onchain" ? `${activeCount} active` : "Placeholder fallback"}</StatusPill>}
      />
